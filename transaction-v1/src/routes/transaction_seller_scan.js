...
router.post('/api/v1/transaction/seller/scan', validateAuth, [
    body('clientId')
        .trim()
        .notEmpty()
    ...
    try {
        await db.transaction(async (trx) => {
            await trx('device_scans').insert({
                ...
                auth_user_id: userId,
                main_info: mainInfo,
                type: DeviceScanType.TransactionSellerScan,
                main_url: mainUrl,
                created_at: date,
                updated_at: date,
                device_id: deviceId,
            ...
            await trx('transactions').update({
                status: TransactionStatus.SellerMustSubmitPasscode,
                updated_at: date,
...
export { router as transactionSellerScanRouter };