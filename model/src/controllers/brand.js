...
const create = async (req, res) => {
  ...
  try {
    ...
    const checkId = await db.first('name').table('brands').where('name', name);
...
const update = async (req, res) => {
  ...
  try {
    ...
    const checkId = await db('brands').first('id').where('id', id);
    if (!check.existy(checkId)) {
      return helper.showClientBadRequest(res, helper.ERR_EXIST);
    }
    await db('brands')
      .update({ name, image_url: imageUrl || null, updated_at: date })
      .where('id', id);
...
const listAll = async (req, res) => {
  try {
    const listall = await db('brands').select();
...
const detail = async (req, res) => {
  ...
  try {
    ...
    const detailed = await db('brands').first().where('id', id);
...
const list = async (req, res) => {
  try {
    ...
    const listpage = await db('brands').limit(limit).offset(offset);
...
const listPost = async (req, res) => {
  ...
  try {
    ...
    const countRow = await db('brands')
      .count('brands.id', { as: 'count' })
      .first();
